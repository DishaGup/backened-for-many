...
cartTrendifyRouter.patch("/update/:productId", async (req, res) => {
  const { productId } = req.params;
  const product = await CartTrendifyModel.findOne({ _id: productId });
  if (product) {
    if (product.userId == req.body.userId) {
      await CartTrendifyModel.findByIdAndUpdate(productId, req.body);
      res.send({ msg: `Product with ${productId} has been updated!!` });
    } else {
